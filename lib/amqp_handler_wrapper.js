...
return-1}return delay*1000};module.exports=function(channel,clientQueueName,failureQueueName,clientHandler,delayFunction,retryCount,initializer){var errorHandler=function errorHandler(msg){if(!initializer.isInitialized){// Delay in 1 MS to let the queues/exchange/bindings initialize
...
if(!isNaN(retryCount)&&retryCount===msg.properties.headers._retryCount){return channel.sendToQueue(failureQueueName,new Buffer(msg.content),msg.properties)}msg.properties.headers._retryCount+=1;var expiration=(delayFunction||getDefaultDelay)(msg.properties.headers._retryCount);if(expiration<1){return channel.sendToQueue(failureQueueName,new Buffer(msg.content),msg.properties)}var properties={persistent:true,headers:{_originalProperties:msg.properties,// save the original properties.
